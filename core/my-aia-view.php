...
class MY_AIA_VIEW {
	 ...
	 * @param \MY_AIA_CONTROLLER $controller
	 */
	 ...
	 * @param string $template name of template relative to /viewss/
	 * @param string $layout name of layout relative to /viewss/layouts/
	...
	public function render($template, $layout='default', $do_output=TRUE) {
		...
		$template	= sprintf('%sviews/%s.ctp', MY_AIA_PLUGIN_DIR, $template); // path to absolute dir
		$layout		= sprintf('%sviews/layouts/%s.ctp', MY_AIA_PLUGIN_DIR, $layout); // path to absolute dir
	...
	public function element($name, $vars=NULL) {
		$name	= sprintf('%sviews/elements/%s.ctp', MY_AIA_PLUGIN_DIR, $name); // path to absolute dir
