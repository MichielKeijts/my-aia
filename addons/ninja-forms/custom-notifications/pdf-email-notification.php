 ...
 * MODIFIED: now sends to group leaders email 
 *
 * @package     Ninja Forms
 * @subpackage  Classes/Notifications
 * @copyright   Copyright (c) 2014, WPNINJAS, 2016 Michiel Keijts
 * @license     http://opensource.org/licenses/gpl-2.0.php GNU Public License
 * @since       2.8
*/

class NF_Notification_Email_PDF extends NF_Notification_Base_Type
{
	...
	function __construct() {
		$this->name = __( 'PDF Email Notification', 'my-aia' );
	}
	...
	public function edit_screen( $id = '' ) {
		...
		if ( $id == '' ) {
			...
			$template_id = Ninja_Forms()->notification( $id )->get_setting( 'template_id' );
		}
		...
		if ( '' != $form_id ) {
			...
			$template_id = $this->get_value( $id, 'template_id', $template_id );
		} else {
			$from_name = get_bloginfo( 'name' );
			$from_address = get_bloginfo( 'admin_email' );
			$reply_to = '';
		...
		<script type="text/javascript">
				...
				nf_notifications.tokens['template_id'] = <?php echo json_encode( $template_id ); ?>;
		</script>
	...
	public function process( $id ) {
		...
		$to				= $this->get_group_admin_email(); // get the admin email
		$cc 			= $this->process_setting( $id, 'cc' );
//		$cc             = $cc[0];
		$bcc 			= $this->process_setting( $id, 'bcc' );
//		$bcc 			= $bcc[0];
		
		$template_id	= $this->process_setting( $id, 'template_id' );

		/*
		 * Modification: First Try and create PDF from template
		 */
		$temlate_ids	= explode(',',$template_id);
		$templates = array();
		foreach ($templates as $template_id) {
			$template_id = trim($template_id); // remove spaces
			$templates[] = $this->get_pdf($template_id);
		}
	...
	/**
	 * Returns the current bp_group id
	 * @global wpdb $wpdb;
	 */
	private function get_group_id() {
		global $wpdb;
		// check based on referer : "/events/sportcommunity-groningen-2016-11-16/"
		$ref =filter_input(INPUT_POST, '_wp_http_referer');
		if ( $ref ) {
			if (preg_match('/^\/events\/([a-z0-9\-]*)/', $ref, $matches)) {
				$event = $wpdb->get_results(sprintf("SELECT group_id FROM {$wpdb->prefix}em_events WHERE event_slug='%s'", $matches[1]));
				if (count($event) > 0) return $event[0]->group_id;
			}
		}
	 ...
	 * @global wpdb $wpdb;
	 */
	private function get_group_admin_email($group_id = NULL) {
		global $wpdb;
		if ($group_id == NULL) $group_id = $this->get_group_id ();
		
		// if not found, send to admin!
		if ($group_id) {
			
			$userinfo = $wpdb->get_results(sprintf("SELECT * FROM {$wpdb->prefix}bp_groups_members WHERE is_admin = 1 AND group_id=%d", $group_id));

			if (count($userinfo) > 0) {
				$user = get_user_by('id', $userinfo[0]->user_id);
				if ($user) 
					return $user->user_email;	// Finally return 
			}
		} // group_id set
		
		
		return get_blog_info('admin_email');
	}
	 ...
	 */
	private function get_pdf ($template_id ) {
		$pdf = new MY_AIA_TEMPLATE_CONTROLLER();
		$pdf->TEMPLATE->get($template_id);
		$filename = $pdf->parse($template_id, $this->ID, MY_AIA_INVOICE_DIR);
	}
}

return new NF_Notification_Email_BPGroup();
