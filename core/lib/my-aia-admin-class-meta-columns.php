...
/**
 * @package MY_AIA
 * inherit from WPSEO_Meta_Columns
 */

/**
 * Class MY_AIA Meta Column 
 * (inherit & modified from WPSEO_Meta_Columns
 */
class MY_AIA_META_COLUMNS {
	/**
	 * Holder for the meta columns
	 * array('meta column name', 'TITLE')
	 * @var array 
	 */
	public $meta_columns;
	
	/**
	 * Name of the post type associated
	 * @var string 
	 */
	protected $post_type;
	 ...
	 * @return array
	 */
	/*pulic function column_heading( $columns ) {
	...
	/**
	 * Insert the heading in the third, 4th column 
	 * @param array $columns
	 * @return array modified $column
	 */
	public function column_heading($columns) {
		return 
			array_slice($columns, 0, count($this->meta_columns)) + 
			$this->meta_columns +
			array_slice($columns, count($this->meta_columns));
	}
	...
	public function column_sort( $columns ) {
		...
		foreach ($this->meta_columns as $key=>$val) {
			$columns[$key] = $key;
		}		
	...
	private function filter_order_by( $order_by ) {
		if (array_key_exists($order_by, $this->meta_columns)  &&  $this->is_meta_field_of_model($order_by)) {
			return array(
					'meta_key' => $order_by,
					'orderby'  => 'meta_value',
				);
	...
	/**
	 * Check if the current element is a part of the current model
	 * @param string $field
	 * @return bool
	 */
	private function is_meta_field_of_model($field) {
		$model_name = strtoupper($this->post_type);
		
		if (array_key_exists($field, $this->{$model_name}->fields)) 
			return TRUE;
		return FALSE;
	}
	 ...
	 */
	protected function set_post_type_hooks() {
	 ...
	 */
	protected function is_metabox_hidden( $post_type = null ) {
			return FALSE;
